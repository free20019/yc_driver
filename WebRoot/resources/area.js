...
function changec(v){
	  var str="0";
	  for(i=0;i<v;i++){
	    str+=("_"+(document.getElementById(cs[i]).selectedIndex-1));
	  };
	  var ss=document.getElementById(cs[v]);
	  with(ss){
	    length = 0;
	    options[0]=new Option(opt0[v],opt0[v]);
	    if(v && document.getElementById(cs[v-1]).selectedIndex>0 || !v){
	      if(dsy.Exists(str)){
	        ar = dsy.Items[str];
	        for(i=0;i<ar.length;i++){
	          options[length]=new Option(ar[i],ar[i]);
...
var cs=["cs_province","cs_city"];//三个select的name
var opt0 = ["省份","地级市"];//初始值
...
function _init_areac(){ //初始化函数
  for(i=0;i<cs.length-1;i++){
   document.getElementById(cs[i]).onchange=new Function("changec("+(i+1)+")");
  }
  changec(0);
}